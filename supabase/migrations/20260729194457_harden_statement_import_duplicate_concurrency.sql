...
begin
  ...
  perform pg_advisory_xact_lock(import_lock.lock_key)
  from (
    select distinct hashtextextended(
      concat_ws(
        '|',
        v_user_id::text,
        v_batch.account_id::text,
        item.competence_date::text,
        item.type::text,
        item.amount_cents::text,
