...
begin
  ...
  -- A user can retain their profile after their finance data is reset. In that
  -- case, recover the missing personal workspace instead of treating the old
  -- completion timestamp as a terminal state.
  if exists (select 1 from public.profiles where id = v_user_id and onboarding_completed_at is not null)
    and v_workspace_id is not null then
