...
A implementação entrega conta principal, saldo real, disponibilidade projetada, registro simples, navegação e histórico; lint, testes e build passaram. O ciclo só será declarado concluído após a execução do cenário manual com uma sessão autenticada em um Supabase configurado. Importação, regras de categorização e contexto MEI permanecem fora do escopo; o próximo ciclo é importação OFX/CSV com prévia, deduplicação e inbox de revisão. Open Finance não foi implementado.
