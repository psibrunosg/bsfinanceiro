...
const CATEGORY_KIND_LABEL: Record<string, string> = { expense: "Despesa", income: "Receita" };
const accountTypeLabel = (t: string) => ACCOUNT_TYPE_LABEL[t] ?? t;
const categoryKindLabel = (k: string) => CATEGORY_KIND_LABEL[k] ?? k;
...
type Occurrence = { id: string; fixed_commitment_id: string; description: string; amount: number; due_date: string; status: string; paid_at: string | null };
...
export function FinanceClientPage({ route, cardId }: { route: Route; cardId?: string }) {
  ...
  async function load() {
    ...
    if (route === "card" && cardId) {
      ...
      setCommitments(data || []);
      const { data: occ } = await supabase.rpc("materialize_fixed_commitment_occurrences", { p_workspace_id: ws.id, p_month: monthStart() });
      setOccurrences(occ || []);
    }
    setLoading(false);
  }
  ...
  async function payCommitment(occurrenceId: string, form: FormData) {
    ...
    if (!accountId) { setMessage("Escolha uma conta para pagar."); return; }
    const { error } = await supabase.rpc("pay_fixed_commitment_occurrence", { p_occurrence_id: occurrenceId, p_account_id: accountId, p_paid_on: new Date().toISOString().slice(0, 10), p_idempotency_key: crypto.randomUUID() });
  ...
  if (route === "commitments") { const expenseCategories = categories.filter(c => c.kind === "expense"); const totalFixed = commitments.reduce((s, c) => s + Number(c.amount), 0); return <main className="management-page">{header("Compromissos", "Contas fixas do mês.")}{nav}{notice}<section className="management-grid"><List title={`Compromissos fixos · ${money(totalFixed)}/mês`}>{commitments.map(c => { const cat = categories.find(x => x.id === c.category_id); return <article className="account-row" key={c.id}><span>📌</span><div><strong>{c.description}</strong><small>vence dia {c.due_day}{cat ? ` · ${cat.name}` : ""}</small></div><b>{money(c.amount)}</b></article>; })}</List><aside className="form-card"><h2>Novo compromisso</h2><SimpleForm onSubmit={submitCommitment}><input name="description" placeholder="Ex.: Aluguel" maxLength={100} required/><input name="amount" placeholder="0,00" required/><input name="due_day" type="number" min="1" max="31" defaultValue="10" required/><select name="account_id"><option value="">Conta (opcional)</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select><select name="category_id"><option value="">Categoria (opcional)</option>{expenseCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select><button>Adicionar</button></SimpleForm></aside></section><section className="account-list"><h2>Este mês · {new Intl.DateTimeFormat("pt-BR", { month: "long", year: "numeric" }).format(new Date())}</h2>{occurrences.length === 0 ? <p className="muted">Nenhuma ocorrência neste mês.</p> : occurrences.map(o => { const paid = o.status === "paid"; const payable = o.status === "planned"; return <article className="account-row" key={o.id}><span>{paid ? "✅" : "🗓️"}</span><div><strong>{o.description}</strong><small>vence {date.format(new Date(`${o.due_date}T12:00:00`))}{paid ? " · paga" : payable ? "" : ` · ${o.status}`}</small>{payable && <form className="finance-form" onSubmit={async e => { e.preventDefault(); const f = new FormData(e.currentTarget); await payCommitment(o.id, f); }}><select name="account_id" required defaultValue=""><option value="">Pagar com...</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select><button>Pagar</button></form>}</div><b>{money(o.amount)}</b></article>; })}</section></main>; }
  ...
  return <main className="dashboard-shell"><section className="hero-card"><div className="hero-brand">{/* eslint-disable-next-line @next/next/no-img-element */}<img className="brand-logo" src={appPath("/logo-bsfinanceiro.png")} alt="BS Financeiro" width={52} height={52} /><div><p className="eyebrow">BS Financeiro</p><h1>Seu painel financeiro</h1></div></div>{nav}</section><section className="summary-grid"><article><span>Saldo inicial</span><strong>{money(balance)}</strong></article><article><span>Cartões</span><strong>{cards.length}</strong></article><article><span>Limite total</span><strong>{money(totalCards)}</strong></article></section><section className="management-grid"><List title="Cartões">{cards.map(c => <article className="account-row" key={c.id}><span className="brand-badge"><BrandLogo brand={c.brand}/></span><div><strong>{c.name}</strong><small>Vence dia {c.due_day}</small></div><b>{money(c.credit_limit)}</b></article>)}</List><List title="Movimentações recentes">{transactions.slice(0,8).map(t => <article className="account-row" key={t.id}><span>{t.type === "income" ? "↑" : "↓"}</span><div><strong>{t.description}</strong><small>{t.competence_date}</small></div><b>{money(t.amount)}</b></article>)}</List></section></main>;
}
