...
export function DashboardChart({ type, labels, values, label, color }: Props) {
  ...
  useEffect(() => {
    ...
    const chart = new Chart(ref.current, { type, data: { labels, datasets: [{ label, data: values, backgroundColor: type === "doughnut" ? values.map((_, index) => `hsl(${158 - index * 24} 55% ${42 + index * 6}%)`) : color, borderColor: color, borderWidth: type === "line" ? 2 : 0, fill: type === "line", tension: .35 }] }, options: { maintainAspectRatio: false, plugins: { legend: { display: type === "doughnut", labels: { color: "#60716c" } }, tooltip: { callbacks: { label: (item) => `${item.dataset.label}: ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(Number(item.raw))}` } } }, scales: type === "doughnut" ? {} : { x: { ticks: { color: "#60716c", maxTicksLimit: 7 }, grid: { display: false } }, y: { ticks: { color: "#60716c", callback: (value) => `R$ ${value}` }, grid: { color: "rgba(96,113,108,.18)" } } } } });
